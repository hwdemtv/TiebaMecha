...
> 💡 **核心优势**：通过对数正态分布延迟、昼夜节律权重、语义级内容混淆、代理预热期等十重反风控技术，最大程度模拟真人行为，降低被系统检测和封禁的风险。
...
- **全域战略吧库**：跨账号贴吧聚合视图，支持搜索/封禁筛选/删帖筛选、分页浏览、批量切换火力目标、批量补齐关注、批量取消关注、修改分组标签
- **本土作战自动判定**：根据封禁和删帖记录自动关闭危险贴吧的发帖许可，恢复需人工确认，避免反复震荡

### 2. 拟人化风控引擎 (BionicDelay™)
- **对数正态分布延迟**：拒绝机械的固定间隔，发帖延迟遵循对数正态分布（大量短停顿 + 偶尔长停顿），极致模拟真实人类操作节奏
- **昼夜节律权重**：根据现实时间（如深夜 01:00 - 07:00）自动调整操作频率，符合生物作息规律
- **周末/工作日差异化**：周末活跃窗口更长、延迟更低；工作日午休和下班后有小高峰，更贴近真实用户行为模式
...
- **浏览伪装**：签到前随机浏览帖子（20% 概率），避免纯签到行为被识别
- **统计追踪**：记录签到历史、连续天数、等级变化
- **自动同步**：从账号自动拉取关注的贴吧列表

### 6. 拟人化养号 (BioWarming™) [ENHANCED]
- **深度模拟**：自动模拟真人进入贴吧浏览、翻页、停顿阅读。
- **搜索行为模拟**：15% 概率模拟搜索关键词，模拟真实用户探索行为。
- **翻页浏览**：30% 概率翻页 2-3 页，增加浏览深度多样性。
- **对数正态停顿**：停顿间隔遵循对数正态分布，大量短停顿 + 偶尔长停顿。
...
|------|------|
| **指挥中心** | 系统概览、账号状态、快捷操作 |
| **账号列表** | 多号导入、存活验证、代理绑定、设备指纹管理、全域战略吧库（跨账号贴吧聚合、火力目标管理、批量操作、分页浏览） |
...
| 延迟分布 | 对数正态分布（大量短停顿 + 偶尔长停顿） | 启用 |
| 生理节律 | 凌晨 1-7 点延迟自动增加 1.5-2.2 倍 | 启用 |
| 周末差异化 | 周末活跃窗口更长、延迟更低 | 启用 |
| 相似度窗口 | 内容去重检测时间窗口 | 24 小时 |
| 代理预热期 | 新绑定代理的发帖限制期 | 48 小时 |

> ⚠️ **安全发帖建议**：单账号日发帖建议 **≤ 5 帖**，签到间隔 **≥ 30 秒**，发帖间隔 **≥ 3 分钟**。超过此频率会显著增加封号风险。多账号矩阵场景下，建议各账号错开操作时段，避免同一时间段内多个账号集中发帖。
...
│   │   ├── ai_optimizer.py  # AI SEO 优化器（6 种人格预设 + 自动轮换）
...
TiebaMecha 内置十重反检测机制，最大程度模拟真人行为、降低被平台风控的风险：

### 1. 拟人化延迟 (BionicDelay™)
- **对数正态分布**：延迟时间遵循对数正态分布，产生大量短停顿 + 少量长停顿，拒绝机械的固定间隔
- **生理节律**：凌晨 1-7 点自动增加 1.5-2.2 倍延迟，模拟真人作息
- **周末/工作日差异化**：周末活跃窗口更长、延迟更低；工作日午休和下班后有小高峰
- **边界保护**：自动裁剪异常值，避免极端延迟

### 2. 语义级内容混淆 (Obfuscator)
- **零宽字符注入**：在标题和正文中注入不可见零宽字符，绕过哈希指纹检测
- **语序打乱 (Semantic Shuffling)**：段落内随机交换相邻句子顺序（跳过含连接词的句子），对抗语义指纹分析
- **间距人性化**：智能调整文本间距，模拟人工输入习惯
- **随机符号注入**：50% 概率在文本边界插入表情符号，增加内容多样性
- **密度可调**：支持自定义混淆密度（0.1-0.5）

### 3. 渐进式熔断器
- **验证码熔断**：检测到验证码后自动暂停该账号 30 分钟
- **渐进式失败熔断**：连续失败 5 次触发熔断，24h 内重复触发逐级加重（30min → 2h → 6h），避免一刀切误杀
- **贴吧级熔断**：目标贴吧连续失败时自动标记并跳过

### 4. 内容相似度检测
- **24 小时窗口**：基于 bigram Jaccard 相似度，检测最近 24 小时内的历史内容，防止跨时段重复
- **自动拦截**：相似度超过阈值（默认 0.7）时自动跳过该物料
...
### 6. 代理预热期
- **48 小时预热**：新绑定代理的账号在预热期内只允许签到/浏览等低风险操作，不允许发帖
- **防止 IP 突变**：避免新代理直接发帖触发百度关联检测

### 7. AI 人格自动轮换
- **按时段自动选择**：白天使用轻松人格（随缘路人、热心网友），晚上使用专业人格（技术老炮、资源大神），深夜使用高冷人格
- **风格多样性**：避免同一账号长期使用同一 AI 风格被识别

### 8. 签到行为伪装
- **浏览前置**：签到前 20% 概率随机浏览 1 个帖子，避免纯签到 API 调用序列被检测
- **矩阵签到隔离**：账号切换间 30-120 秒随机延迟

### 9. 养号行为增强
- **搜索模拟**：15% 概率模拟关键词搜索行为
- **翻页浏览**：30% 概率翻页 2-3 页，增加浏览深度
- **对数正态停顿**：所有停顿间隔遵循对数正态分布
...
| ✅ | 拟人化风控引擎 | 对数正态分布延迟、昼夜节律、周末/工作日差异化 |
| ✅ | AI 智能改写 | 智谱/DeepSeek/OpenAI，6 种人格预设 + 时段自动轮换 |
| ✅ | 自动回帖 (Auto-Bump) | 帖子热度维护 |
| ✅ | 拟人化养号 (BioWarming™) | 浏览/搜索/翻页模拟、随机点赞、新号冷启动破冰 |
| ✅ | 帖子存活分析 | 存活追踪、死亡原因分类、多维筛选 |
| ✅ | 战术情报分析 | 拦截原因解析、实战应对建议 |
| ✅ | 全域通知中心 | 本地 + 远程通知 |
| ✅ | 自动更新检测 | GitHub Releases |
| ✅ | 全域签到引擎 | 批量签到、浏览伪装、跨账号矩阵签到、贴吧同步 |
| ✅ | 行为审计模块 | 风险评分、异常检测、改进建议 |
| ✅ | 代理预热期 | 新代理 48h 预热，防止 IP 突变关联 |
| 🔜 | 云控中心 | 多节点统一管理 |
| 🔜 | 数据分析仪表盘 | 可视化统计报表 |
| 🔜 | 移动端适配 | 响应式 Web UI |
