...
> - ✅ 任务中心独立页（380d8ba）：新增 `web/pages/batch_post_center.py`（任务队列/已发归档/运行日志 + 复制配置跨页交接）；配置页瘦身 -1180 行，保留物料视图与执行监视区；新增 `batch_post/log_stream.py` LogStream 流水单源组件（配置页/运行中心共用）；app.py 侧栏"执行中心"组新增"发帖运行中心"入口。
> - ✅ 四步向导化（f316d75）：三栏布局重组为 选择账号与目标 → 准备物料 → 策略与排期 → 确认发射；自绘步骤条 + 前跳门禁（未锁贴吧/未选账号不可前进）；启动/干跑按钮移入步骤 4，启动时自动预检并弹摘要确认。
>
> 注意：新增控件需同步补 `tests/test_log_view.py` 的假 ft 桩成员（本次补了 `ft.alignment`）；流水筛选 `key` 值匹配逻辑在 `LogStream.matches_filter`；桩不保留控件子树，向导步骤样式代码做了防御式处理。
